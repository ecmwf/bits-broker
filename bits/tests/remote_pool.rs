...
async fn worker_requests_redirect() {
    ...
    let done = client
        .post(format!(
            "http://127.0.0.1:{port}/test_pool/complete/redirect/{job_id}"
        ))
        .json(&serde_json::json!({
...
async fn complete_unknown_job_returns_404() {
    ...
    let resp = Client::new()
        .post(format!(
            "http://127.0.0.1:{port}/test_pool/complete/reject/no-such-job"
        ))
...
async fn remote_pool_preserves_cost_weighted_ordering() {
    ...
    client
        .post(format!(
            "http://127.0.0.1:{port}/test_pool/complete/reject/{blocker_id}"
        ))
    ...
    client
        .post(format!(
            "http://127.0.0.1:{port}/test_pool/complete/reject/{}",
