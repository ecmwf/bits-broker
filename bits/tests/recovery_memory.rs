...
async fn threshold_persistence_and_cleanup() {
    ...
    let handle = broker
        ...
        .submit(Job::new_with_id(
            new_recovery_job_id("tst", "mem", 30),
            json!({"kind": "cleanup"}),
        ))
...
async fn fast_jobs_do_not_persist() {
    ...
    let handle = broker
        .bits
        .submit(Job::new_with_id(
            new_recovery_job_id("tst", "mem", 31),
            json!({"kind": "fast"}),
        ))
        .expect_accepted("submit should not be rejected");
    let result = wait_for_ready(&broker.bits, &handle.id, Duration::from_secs(1)).await;
...
async fn active_lease_proxies_success_result() {
    ...
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 1);
    let job_id = new_recovery_job_id("tst", "mem", 1);
...
async fn active_lease_proxies_redirect_result() {
    ...
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 2);
    let job_id = new_recovery_job_id("tst", "mem", 2);
...
async fn active_lease_proxies_error_result() {
    ...
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 3);
    let job_id = new_recovery_job_id("tst", "mem", 3);
...
async fn active_lease_proxies_gone_as_cancelled() {
    ...
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 4);
    let job_id = new_recovery_job_id("tst", "mem", 4);
...
async fn active_lease_proxies_not_found() {
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 5);
    let job_id = new_recovery_job_id("tst", "mem", 5);
...
async fn owner_404_without_durable_record_is_not_found() {
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 6);
    let job_id = new_recovery_job_id("tst", "mem", 6);
    let owner_url = start_not_found_owner_stub().await;
    store
        .upsert_broker_lease(owner_id, &owner_url, Duration::from_secs(2))
        .await
...
async fn active_lease_prevents_reclaim_when_proxy_fails() {
    ...
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 8);
    let job_id = new_recovery_job_id("tst", "mem", 8);
...
async fn expired_lease_enables_reclaim_and_completion() {
    ...
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 9);
    let job_id = new_recovery_job_id("tst", "mem", 9);
...
async fn recently_expired_lease_stays_pending_within_clock_skew_buffer() {
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 10);
    let job_id = new_recovery_job_id("tst", "mem", 10);
...
async fn expired_lease_without_record_is_job_lost() {
    ...
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 16);
    store
        .upsert_broker_lease(
            owner_id,
            "http://127.0.0.1:1/job",
    ...
    let job_id = new_recovery_job_id("tst", "mem", 16);
    assert!(matches!(
        claimant
            .bits
            .poll(&job_id, Some(Duration::from_millis(250)))
...
async fn competing_claimants_only_one_wins() {
    ...
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 11);
    let job_id = new_recovery_job_id("tst", "mem", 11);
...
async fn lease_lookup_backend_failure_returns_pending() {
    ...
    .await;
    let job_id = new_recovery_job_id("tst", "mem", 15);
...
async fn backend_claim_errors_backoff_within_single_poll() {
    ...
    .await;
    let job_id = new_recovery_job_id("tst", "mem", 14);
...
async fn proxy_auth_error_returns_pending() {
    ...
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 12);
    let job_id = new_recovery_job_id("tst", "mem", 12);
...
async fn proxy_throttled_returns_pending() {
    ...
    let store = shared_store().await;
    let owner_id = broker_identity("tst", "mem", 13);
    let job_id = new_recovery_job_id("tst", "mem", 13);
...
async fn cancel_not_preserved_across_reclaim() {
    ...
    let store = shared_store().await;
    let claimant_id = broker_identity("tst", "mem", 32);
    let owner_id = broker_identity("tst", "mem", 17);
    let job_id = new_recovery_job_id("tst", "mem", 17);
