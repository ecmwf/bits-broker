...
```yaml
bits:
  persist_after_secs: 10.0   # persist jobs still in-flight after this many seconds
```

| Field | Purpose |
|-------|---------|
| `persist_after_secs` | Threshold before a job is written to durable storage. Jobs completing before this threshold are never persisted. |
